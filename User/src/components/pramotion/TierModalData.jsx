...
const TierModalData = ({ initialTier = 'All', onTierConfirm }) => {
  ...
  const calculateTransform = (tier) => {
    const tierIndex = ['All', 'Tier 1', 'Tier 2', 'Tier 3', 'Tier 4', 'Tier 5', 'Tier 6'].indexOf(tier);
  ...
  return (
    <div>
      ...
      <div className="bg-gray-200 rounded-lg p-2 mx-3 mt-3" onClick={handleToggleModal}>
      ...
      <TierModal isOpen={isModalOpen} onClose={handleToggleModal} confirm={handleConfirmTier}>
        ...
        <div className="absolute top-1/2 -translate-y-full w-full border-t border-gray-100"></div>
        <div className="absolute top-2/4 translate-y-full w-full border-t mt-10 border-gray-100"></div>
          <div
            className="flex flex-col items-center space-y-1 text-16px mt-10 transition-transform duration-300"
            style={{
